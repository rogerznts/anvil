...
conflito. O ticket 09 conferiu na prática, com o omp 18.3.0: dois tickets numa
chamada do `task`, os dois com `isolated: true`, cada implementer mudando a mesma
linha do mesmo arquivo e commitando.

- O omp integra cada implementer quando ele termina, na ordem em que terminam, e
  não na ordem dos itens. O primeiro a terminar entrou no branch da spec.
- O cherry-pick do segundo conflitou. O omp o desfez (`cherry-pick --abort`): a
  árvore ficou limpa, sem `CHERRY_PICK_HEAD`, sem stash e sem marca de conflito. O
  resultado do `task` trouxe "Branch merge failed: omp/task/<id>" e "The unmerged
  branch remains for manual resolution".
- O ticket do segundo ficou com o `Status:` de antes no branch da spec, e o
  `frontier.sh` o devolveu no frontier. O trabalho dele ficou no `omp/task/<id>`.
- Lido no fonte do omp (`src/task/worktree.ts`), não observado: o omp guarda a
  árvore suja com `git stash` antes do cherry-pick e a devolve depois. Só um
  conflito nessa devolução deixa a árvore suja, e o omp avisa. A `anvil-run` só
  despacha com a árvore limpa, por isso esse caso não foi reproduzido.
...
cherry-pick conflitou não muda de estado e sai como pulado nesta execução. A
seguinte o despacha de novo, a partir do branch da spec, que já tem o trabalho do
ticket que entrou. Se a árvore ficar suja, a parada de sempre, o `halt`, pega.
...
e o `ready` mostra isso. O branch `omp/task/<id>` do ticket que conflitou fica no
repositório com o trabalho dele, e os commits da leva entram no branch da spec na
ordem em que os implementers terminam. O `sha` que o implementer grava na linha
