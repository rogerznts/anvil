...
- Depois da revisão, no ticket 05: a pasta da camada no payload passou de `omp-layer/` a `.omp-layer/`, porque o Codex acha `SKILL.md` em subpasta de skill e só pula pasta oculta. Os comentários acima citam o nome antigo. O P3 sobre a frase de remoção da `anvil-update` fechou no mesmo ticket.
