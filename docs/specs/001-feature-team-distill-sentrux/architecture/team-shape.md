...
| Despacho | parâmetros da chamada `Agent` ([5](#5-delegação)); nomes; modelo pela lista `team` ([10](#10-lista-team)); despachar ou retomar, e papel despachado com worktree nunca se retoma por `SendMessage` ([5](#5-delegação)); escrita: o Tester e o Review não contam como escritores, porque escrevem fora de qualquer checkout, num diretório que o Leader cria com `mktemp -d` a cada despacho e nomeia por caminho absoluto; o que o Mission Control tem e o papel precisa vai **copiado** para o Contexto; nota de que tudo nesta seção foi medido no Claude Code 2.1.270 e se reverifica por versão |
...
| O retorno | a última mensagem é o retorno e chega a quem despachou; primeira linha fixa; seções; severidade de finding ([6](#6-retorno-e-gates)); mensagem final sem palavra do protocolo não é entrega, e o Leader não a registra e pede o retorno por `SendMessage`, ou por `Agent` novo se o papel roda em worktree; o mesmo retorno emitido de novo, sem trabalho novo, o Leader não registra outra vez |
...
| Ownership de escrita | um escritor por região; a Política de escrita é a fronteira; `Status:` e `## Comments` de ticket são do Leader; sobreposição descoberta no meio → parar e devolver `BLOQUEADO`, e quem escolhe a saída é o Leader; em worktree, conferir que `HEAD` contém o sha da Base antes de escrever, e não conter é `BLOQUEADO`; **em worktree, ficar nele**: logo depois de ler o protocolo, rodar `git rev-parse --show-toplevel` e anotar a saída; no começo de cada turno, inclusive quando uma mensagem o retoma, e antes de cada commit, rodar o mesmo comando e comparar a saída com o anotado; o comando vai **puro, sozinho numa chamada Bash**, sem `cd`; saída diferente, `BLOQUEADO` sem escrever, dizendo que foi retomado fora do worktree. Comando puro porque, num agente com `isolation: "worktree"`, o harness recusa `git` dentro de `$(…)`, `test`, `[ ]`, `T=$(…) && test` e `sh -c` ("names git in a form too complex to verify"), medido no gate Tester R3 do 08 (T3-F1). Começo de turno, e não antes de cada escrita, porque essa forma não foi seguida (0 de 4 Edits, T3-F2) e porque a retomada por mensagem é sempre um turno novo. Prova dentro de agente isolado (`workspace/25-team-guard-pure`): o comando puro rodou sem recusa, saiu o caminho do worktree no despacho e o do checkout na retomada por `SendMessage`, com o worktree sem mudança já apagado. `--show-toplevel`, e não `--git-dir` sob `worktrees/`, porque essa variante passa no worktree do outro Dev (medido num repo descartável) |
...
| Perguntas e autonomia | decisão do usuário (intenção de produto, preferência, requisito ausente, trade-off de negócio, informação indisponível, ação irreversível) vira `PERGUNTA`, e a resposta volta por `SendMessage`, na mesma sessão, ou num `Agent` novo se o papel roda em worktree; o que se descobre lendo, rodando, pesquisando ou perguntando a um colega não vira; trabalho reversível avança com evidência suficiente |
...
- **Autor** (PO, Architect, Analyst, Designer, Dev) com mais trabalho no mesmo
  ...
  finding, resposta de `PERGUNTA`, próxima pergunta do grill. **Exceto papel
  despachado com worktree**, que nunca se retoma por `SendMessage` (ver "Papel
  despachado com worktree", abaixo).
...
**Papel despachado com worktree.** Nunca se retoma por `SendMessage`: nem para
correção de finding depois da integração, nem para responder `BLOQUEADO` ou
`PERGUNTA`. Vai `Agent` novo com o mesmo name, sobre o branch da spec já integrado,
com a resposta ou a correção no Contexto, e o Contexto apontando o ticket e os
comentários.

- Se o worktree dele sobreviveu, o Contexto aponta os commits que
  `git cherry {branch-da-spec} worktree-agent-{id}` marca com `+`, e o papel traz só
  esses por `cherry-pick` antes de continuar. Os `-` já estão integrados; o intervalo
  desde a Base os repetiria, e o `cherry-pick` para no commit já integrado, que fica
  vazio (medido). Esse worktree sai pelo passo 8 da seção 11, quando `git cherry` não
  listar `+`.
...
Por quê, medido no gate Tester do 08 (o achado F1, Dev em worktree commitando no
branch da spec):

- **Sem mudança**, o worktree é apagado com o branch quando o agente encerra, e uma
  `SendMessage` a ele o retoma no cwd de quem manda. O Dev que voltou `PERGUNTA` sem
  commit, retomado pelo Leader, commitou direto no branch da spec.
- **Com commit**, a retomada fica no worktree. Mas a `SendMessage` não distingue os
  dois casos, então a regra não depende de qual deles aconteceu.
- Depois da integração, o branch do worktree não tem os commits do outro escritor, e
  a correção cairia numa base velha.
- `isolation` do `Agent` só aceita `"worktree"`, sem caminho (pelo schema da
  ferramenta), então um `Agent` novo não volta a um worktree que já existe.

Por isso remover o worktree logo depois da integração, antes do veredito dos gates,
não custa nada: a correção nunca voltaria para ele.

**A regra do Leader não alcança a lateral.** Um papel em worktree retomado por
`SendMessage` de outro papel também roda no cwd de quem manda. Quem cobre esse caso
é a guarda do lado do papel, em PROT § Ownership de escrita: o papel anota o
`git rev-parse --show-toplevel` do worktree ao começar e confere no começo de cada
turno e antes de cada commit, com o comando puro, e fora dele volta `BLOQUEADO` sem
escrever (RV2-F1 do
Review R2 do 08, a guarda que vale para qualquer remetente).
...
nomeia por caminho absoluto um diretório fora de qualquer checkout e diz
`Commit: não`. O Leader cria um diretório por despacho, com
`mktemp -d "${TMPDIR:-/tmp}/anvil-{NNN}-{name}.XXXXXX"`: um caminho fixo como
`/tmp/anvil-001-tester-01` se repete entre projetos e guarda restos de outros
despachos (gate Tester do 08, F3). O Dev é o único escritor do checkout. Dois motivos: dois escritores
...
- **Retomada por lateral.** Papel retomado por `SendMessage` de **outro papel** roda
  no cwd de quem manda e, se foi despachado sem `model`, no modelo de quem manda; com
  `model` explícito, fica no dele. Medido no gate Tester do 08 (2 de 2): o
  `tester-01`, retomado pelo `dev-01`, rodou no modelo do Dev, com o cwd no worktree
  do Dev. Por isso a Política do Tester nomeia só caminho absoluto, e o agente do
  Tester, § Onde você escreve, diz: "Retomado por `SendMessage` de outro papel, o seu
  cwd pode ser o worktree do autor. Continue escrevendo só no caminho absoluto que a
  Política de escrita nomear."
...
## Política de escrita
Leitura no checkout, escrita só em \`{caminho absoluto do diretório criado pelo Leader}\`, onde você extrai o commit que testa com \`git archive {head} | tar -x -C {diretório}\`.
Commit: não.
...
commit num diretório fora do checkout, criado pelo Leader como o do Tester (§5), e
não mexe no `HEAD`, no índice nem no working tree. A allowlist e o PROTOCOL não mudam: "o Review não edita" é sem `Edit` e
...
**Com escritor ativo no checkout, o Leader não escreve.** Nem no ticket nem no
Mission Control: guarda o Registro e a decisão (de entrega ou de gate) e os escreve
quando o escritor voltar. O commit do escritor stagearia o que estivesse pendente
(`anvil-implement` e `tea-commit` stageiam tudo). O Mission Control entra na regra
porque se commita do mesmo jeito e o passo 8 da seção 11 o edita (RV2-F4 do Review
R2 do 08).
...
5. Quando os dois voltam, o Leader integra um de cada vez no branch da spec:
   ...
   `PRONTO` (passos 5 a 8 para ele), e o outro segue a regra do §5, "Papel
   despachado com worktree". Enquanto esse escritor único escreve no checkout, o
   registro dos gates do `PRONTO` e a tabela do passo 8 esperam a volta dele (§8,
   "Com escritor ativo no checkout, o Leader não escreve").
...
8. Remove cada worktree com `git worktree remove {caminho}` e o branch com
   ...
   remover. Worktree que sumiu sozinho (o papel encerrou sem mudança) não aparece em
   `git worktree list`, e não há o que remover. Removido um worktree, ou sumido
   sozinho, o Leader atualiza a tabela "Trabalho em execução e
...
não ao worktree (§5, "Papel despachado com worktree"). Mecânica medida no 2.1.270:
sem mudança, o worktree é apagado com o branch quando o agente encerra, e uma
`SendMessage` a ele, de qualquer remetente, o retoma no cwd de quem manda.
...
- **M5. Medido no 08.** O worktree de `isolation: "worktree"` parte do `HEAD` do
  ...
  11. Medido depois, no gate Tester do 08: sem mudança, o worktree é apagado com o
  branch quando o agente encerra, e `SendMessage` a ele o retoma no cwd de quem
  manda; papel retomado por lateral roda no cwd e, sem `model`, no modelo de quem
  manda.
