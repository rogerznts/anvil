...
Run the **anvil-arena** skill with the design-sketch task and the Phase A grounding artifacts. Pass `references/runner-prompt.md` as each runner's prompt. Each candidate produces a design package shaped per `references/rationale-template.md`: the caller's usage written first, then the type sketch, function signatures, module map, and prose rationale derived from it.

Use your configured architect runners from `.claude/rules/anvil.md` (defaults `opus`, `fable`, `sonnet`).
...
The synthesis can ship as its own commit either way. That's the "scaffold first" mode — build the skeleton before the flesh; subsequent commits read as filling in bodies against a stable contract. Planned and scoped breakage during fill-in is fine, as long as the outcome is the thing being tracked. For adversarial pressure on the design before implementing, run the **anvil-code-review** skill on the synthesized sketch: its two independent axes are the closest thing to a hostile read.
