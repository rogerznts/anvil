...
1. Locate the prior trail. A local transcript under the active workspace's `agent-transcripts/` directory (the system prompt names the path; do not glob across `~/.claude/projects/*/`, that crosses workspace boundaries and reads private chats from unrelated projects), a cloud-agent URL, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **anvil-principles** skill, `references/guard-the-context-window.md`).
...
5. Verify the inherited claims against the original goal on the real artifact (the **anvil-principles** skill, `references/prove-it-works.md`). A passing prior self-report is not the proof.
