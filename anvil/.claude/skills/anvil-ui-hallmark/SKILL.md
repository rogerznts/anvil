...
The per-theme axis values live as comments at the top of each theme's tokens block in [`references/themes.css`](references/themes.css). When in doubt, name your candidate theme out loud and identify its three axis values; if two of three match the previous output, redirect.
