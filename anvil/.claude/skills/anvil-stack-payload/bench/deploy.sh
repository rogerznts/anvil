#!/usr/bin/env bash
# deploy.sh — driver de deploy do adapter Payload (provedor: Railway).
#
# Publica uma ferramenta criada pelo /anvil-bench num servidor Railway, usando a
...
#
# Suporta --help e --dry-run. Assume conta paga (sem free-tier).
#
...
usage() {
    cat <<'EOF'
Usage: deploy.sh [--name <nome-publico>] [--dry-run] [--help]

Publica no Railway a ferramenta Payload do diretório atual (projeto /anvil-bench).

Pré-requisitos:
  - Rodar dentro de um projeto criado pelo /anvil-bench (starter Payload presente).
...
recognize_project() {
    if [[ ! -f "$PROJECT_DIR/src/payload.config.ts" || ! -f "$PROJECT_DIR/package.json" ]]; then
        echo "ERRO: este diretório não parece uma ferramenta criada pelo /anvil-bench" >&2
...
generate_overlay() {
    ...
    cat > "$df" <<'DOCKER'
# Dockerfile.production — gerado por deploy.sh. Buildado REMOTAMENTE no
...
if $DRY_RUN; then
    ...
    echo "O ambiente local (/anvil-bench) continua funcionando separado, sem alteração."
fi
