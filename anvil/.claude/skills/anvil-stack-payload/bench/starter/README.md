# payload-starter (golden starter do modo `/anvil-bench`)

Starter Payload+Docker **versionado** que já **sobe e loga** antes de qualquer
customização (ADR-0003). O modo `/anvil-bench` copia esta pasta **as-is** para
`~/projects/<nome-projeto>` e só então customiza `src/collections/` e labels.

> Este README é para **quem mantém o anvil**, não para o usuário leigo. O leigo
...
- `docker-compose.yml` — só o serviço `app` (Payload/Next), entra na `anvil-net`.
- `.anvil-infra/docker-compose.yml` — Postgres + Redis + rede `anvil-net`
  (infra compartilhada por máquina, gerida por `infra.sh`).
...
#    O compose da infra é dono da rede `anvil-net` (não-external) e a cria sozinho.
#    NÃO rode `docker network create anvil-net` antes — pré-criar a rede colide com
#    o compose ("network ... has incorrect label") e o Postgres não sobe.
docker compose -f .anvil-infra/docker-compose.yml up -d
...
- As versões em `package.json` estão **pinadas exatas** (sem `^`/`~`) para subida
  ...
  revisada** aqui no `anvil/` (ADR-0003).
